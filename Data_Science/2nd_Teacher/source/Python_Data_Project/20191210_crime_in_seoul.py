...
crime_anal_raw['검거']=np.sum(crime_anal_raw[col], axis=1) # axis=1(열-방향) 행 연산
crime_anal_raw.head()

col2=['강간 발생','강도 발생','살인 발생','절도 발생','폭력 발생']
crime_anal_raw['범죄']=np.sum(crime_anal_raw[col2], axis=1) # axis=1(열-방향) 행 연산
crime_anal_raw.head()

map_station =folium.Map(location=[37.5502, 126.982],
                        zoom_start=11)

map_station.choropleth(geo_data = geo_str,
               data = crime_anal_norm['범죄'],
               columns = [crime_anal_norm.index, crime_anal_norm['범죄']],
               fill_color = 'PuRd', # PuRd, YIGnBu
               key_on='feature.id')
...
for n in crime_anal_raw.index: # 행 인덱스 반복 출력
    folium.CircleMarker([crime_anal_raw['lat'][n], crime_anal_raw['lng'][n]],
                        radius = crime_anal_raw['검거'][n]*0.01,
                        color = '#3186cc', 
                        fill_color='#3186cc',
                        fill=True).add_to(map_station)
    
    folium.Marker([crime_anal_raw['lat'][n], crime_anal_raw['lng'][n]],
                  popup=[crime_anal_raw['관서명'][n],crime_anal_raw['검거'][n]], # 경찰서 이름 아이콘에 표시
                  icon=folium.Icon(color='red',icon='info-sign')).add_to(map_station)
